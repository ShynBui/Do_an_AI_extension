...
from saleapp import app, db, question_answerer, model_multiple, summarizer, rdrsegmenter
import json
...
def answer_question_ask2(query):
    docs = retriever.get_relevant_documents(query.lower(), search_type="mmr", search_kwargs={"k": 3})
    query = ''.join(rdrsegmenter.word_segment(query))
    print(query)
    print(docs[0])
    context = docs[0].page_content.lower()
    context = ''.join(rdrsegmenter.word_segment(context))
